...
16. **Defer `SocialProof` + `ExitIntent` + `CursorCompanion` behind `requestIdleCallback`** — `ClientShell.tsx` already uses `next/dynamic({ ssr: false })` which ships the JS at hydrate time. These components don't need to be ready for several seconds. Wrap mounts in `requestIdleCallback` (with a `setTimeout` fallback) to push their JS parse cost off the interactive window. Rationale: these are "delight" features; none of them should contend with the main thread during the user's first scroll.
