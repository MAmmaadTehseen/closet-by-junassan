...
export default function CartDrawer() {
  ...
  return (
    <Drawer open={open} onClose={close} title={t("cart.title")}>
      {items.length === 0 ? (
        <div className="flex h-full flex-col items-center justify-center gap-4 p-10 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-cream">
            <ShoppingBag className="h-7 w-7 text-muted-foreground" />
          </div>
          <p className="font-display text-lg font-semibold">{t("cart.empty_title")}</p>
        ...
        <div className="flex h-full flex-col">
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">{t("cart.size_label")} {item.size}</p>
          ...
          <div className="border-t border-border bg-cream/50 p-5">
            ...
            <p className="mb-4 flex items-center gap-2 text-[11px] uppercase tracking-widest text-muted-foreground">
              <span className="h-1.5 w-1.5 rounded-full bg-green-600" /> {t("cart.flat_delivery")}
            </p>
