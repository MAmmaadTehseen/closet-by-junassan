...
export default function ProductCard({
  ...
  return (
    <div className="group relative">
      ...
      {/* Quick actions: wishlist + compare (on image, top-right) and quick-add (inline with price row, always visible). */}
      <div className="absolute right-2 top-2 flex flex-col gap-1.5 sm:right-2.5 sm:top-2.5 sm:opacity-0 sm:transition-opacity sm:duration-300 sm:group-hover:opacity-100 sm:focus-within:opacity-100">
        <WishlistButton productId={product.id} productName={product.name} />
        <CompareButton productId={product.id} productName={product.name} compact />
      </div>
